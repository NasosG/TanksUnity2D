...
namespace Pathfinding {
	...
	public class AILerp : VersionedMonoBehaviour, IAstarAI {
		...
		protected virtual void Update () {
            ...
            //rotate towards enemy if close
            if (Vector3.Distance(GetComponent<AIDestinationSetter>().target.position, transform.position) < 5f) {
                Vector3 dir = GetComponent<AIDestinationSetter>().target.position - transform.position; //direction of the target relatively to the npc       
                float angle = Mathf.Atan2(dir.y, dir.x) * Mathf.Rad2Deg + 270; //angle needed to move towards the target
                transform.rotation = Quaternion.AngleAxis(angle, Vector3.forward); //rotate the npc towards the target
            }
            //move towards enemy if not too close and canMove
            if (canMove && Vector3.Distance(GetComponent<AIDestinationSetter>().target.position, transform.position) > 2f) {
